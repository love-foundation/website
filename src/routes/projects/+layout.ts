...
export const load: LayoutLoad = async () => {
	try {
    const projects = callApi? await directus()
          .items('projects')
          .readByQuery({
            fields: ['id', 'name', 'main_image.*', 'pillar', 'slug', 'location_country', 'content.*.*', 'hero_background_color'],
            filter: {
              status: {
                _in: status
              }
  ...
  if (!projects.data) {throw new Error("No data returned from Directus");}

	const projectsData: ConvertedProjects[] = projects.data.reverse().map((project) => {
		const { id, name, pillar, location_country, main_image, slug, content, hero_background_color} = project;
		return {
			id,
